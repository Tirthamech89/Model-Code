...
df_train = train2 %>%  na.omit() %>% mutate(target_var = factor(target_var,labels = c("Failure", "Success")))
  #..................................... Grid Search of Xgboost ....................................................................
xgb_grid_1 = expand.grid(  nrounds = c(500),  
                           max_depth = c( 4, 6, 8),  
                           eta = c(0.1, 0.01),  
                           gamma = c(1, 2),  
...
xgb_trcontrol_1 = trainControl(  method = "repeatedcv",  
                                 number = 5,  
                                 repeats = 2,  
                                 verboseIter = TRUE,  
                                 returnData = FALSE,  
...
xgb_train_1 = train(  x = as.matrix(df_train %>% select(-target_var)),  
                      y = as.factor(df_train$target_var),  
...
AUC <- function(pred,depvar){require(ROCR)  
                             ...
                             return (auc)}
KS <- function(pred,depvar){require(ROCR)  
                            p <- prediction(as.numeric(pred),depvar)  
...
{    install.packages("dplyr")    library(dplyr)}  
 ...
 gaintable = helper %>% group_by(bucket)  %>% summarise_at(vars(depvar), funs(total = n(), 
